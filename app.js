...
// ============================================================
// ENCRYPTED DATA — Generated by VEX Encrypt Tool
// ============================================================
const _ENC_SERVERS = [
    // ⚠️ اینجا خروجی encrypt-tool.html رو کپی کن
    // { id: 'server1', name: 'سرویس ۱', u: '...', k: '...' },
    // { id: 'server2', name: 'سرویس ۲', u: '...', k: '...' },
    // { id: 'server3', name: 'سرویس ۳', u: '...', k: '...' },
    // { id: 'server4', name: 'سرویس ۴', u: '...', k: '...' },
];

const _ENC_RESET_CODE  = 'REPLACE_WITH_ENCRYPTED';
...
function _decrypt(enc) {
    try {
        let r = enc.split('').reverse().join('');
        let s = atob(r);
        let parts = s.split('::');
        let b = parts[1];
        let x = decodeURIComponent(escape(atob(b)));
        let rev = x.split('').reverse().join('');
        let plain = _xor(rev, _S1);
        return plain;
    } catch (e) {
...
function decryptAll() {
    if (_RUNTIME.decrypted) return;
    try {
        _RUNTIME.servers = _ENC_SERVERS.map(s => ({
            id: s.id,
            name: s.name,
            url: _decrypt(s.u),
            key: _decrypt(s.k)
        }));
        _RUNTIME.resetCode  = _decrypt(_ENC_RESET_CODE);
        _RUNTIME.decrypted  = true;
    } catch (e) {
        console.error('decryptAll failed:', e);
    }
}
...
// ============================================================
const ADMIN_USERNAME = 'vexgramx';
const ADMIN_PASSWORD = 'vexgramx';
const VIP_USERNAME   = 'mogovpn';
const VIP_PASSWORD   = 'mogovpn';
...
const EMAILJS_SERVICE_ID_REG    = 'service_nhmchmd';
const EMAILJS_TEMPLATE_ID_REG   = 'template_0npx9m9';

// ============================================================
// NTFY URLs
// ============================================================
const NTFY_PROFILE  = 'https://ntfy.sh/ghozarshvexpromax';
const NTFY_ORDER    = 'https://ntfy.sh/orderprovexmaxv';
const NTFY_VPN      = 'https://ntfy.sh/vpnvexmaxpcmobilvex';
const NTFY_REGISTER = 'https://ntfy.sh/vextelbotamirvexv';
const NTFY_VERIFY   = 'https://ntfy.sh/brefiysgrppihbex';
const NTFY_CHARGE   = 'https://ntfy.sh/nooovechpiyvwvvvvwvuyt';

async function _ntfy(url, message) {
    try {
...
async function ntfyProfile(m)  { return _ntfy(NTFY_PROFILE, m); }
async function ntfyOrder(m)    { return _ntfy(NTFY_ORDER, m); }
async function ntfyVpn(m)      { return _ntfy(NTFY_VPN, m); }
async function ntfyRegister(m) { return _ntfy(NTFY_REGISTER, m); }
async function ntfyVerify(m)   { return _ntfy(NTFY_VERIFY, m); }
async function ntfyCharge(m)   { return _ntfy(NTFY_CHARGE, m); }
...
const CUSTOM_COMMENT_MAP = {
    server1: { ids: ['19172'], platform: null },
    server2: { ids: ['200', '194'], platform: null },
    server3: { ids: ['5774', '5772'], platform: null },
    server4: { ids: ['994', '2191', '1560', '2975', '3428', '3276'], platform: 'instagram' }
};

const DEFAULT_SERVERS_FALLBACK = [
...
const VPN_PRICING = {
    openvpn: {
        single: { 10: 190000, 15: 230000, 20: 280000, 30: 370000, 40: 450000, 50: 510000 },
        multi:  { 10: 260000, 15: 320000, 20: 380000, 30: 510000, 40: 600000, 50: 690000 },
        unlimited: { '1': 600000, '2': 850000, '10': 1980000 }
    },
    v2ray: {
        single: { 10: 80000, 20: 156000, 30: 228000, 40: 296000, 50: 360000 },
        unlimited: { '1': 280000, '2': 480000 }
    }
};
...
چرا OpenVPN؟
OpenVPN یک پروتکل متن‌باز (Open Source) و فوق‌العاده قدرتمند است که از استانداردهای رمزنگاری نظامی (AES-256) استفاده می‌کند.
...
V2Ray چیه؟
V2Ray یک پلتفرم متن‌باز و بسیار انعطاف‌پذیره که برای عبور از فیلترینگ سنگین طراحی شده.

⚖️ حقیقت‌ها:
• سرعت: ⚠️ متغیر
• پایداری اتصال: ❌ کمی ضعیف
• امنیت: 🔸 خوب
• عبور از فیلترینگ: ✅ قوی‌ترین
• مصرف باتری: ⚠️ سنگین‌تر

✅ مناسب برای: وب‌گردی و شبکه‌های اجتماعی
❌ نامناسب برای: گیمینگ آنلاین، استریم طولانی 4K

🎯 جمع‌بندی: V2Ray = عبور از فیلترینگ قوی، اما پایداری کمتر.`
};
...
let _authState = {
    ...
    registerStep: 1,
    loginStep: 1,
    regData: { username: '', password: '', phone: '', email: '' },
    pendingCode: '',
    codeExpire: 0,
    codeTimer: null,
    pendingAction: null,
...
function getServers() {
    const custom = store.get('vex_servers_custom', null);
    if (custom && Array.isArray(custom) && custom.length > 0) return custom;
    if (_RUNTIME.servers && _RUNTIME.servers.length > 0) return _RUNTIME.servers;
    return DEFAULT_SERVERS_FALLBACK;
}
function saveServers(s) { store.set('vex_servers_custom', s); }

// ============================================================
// USER MANAGEMENT
// ============================================================
function getUsers() { return store.get('vex_users', []); }
function saveUsers(users) { store.set('vex_users', users); }
...
function setActiveUser(username) { store.set('vex_active_user', username); }
function getActiveUser() {
    const username = store.get('vex_active_user', null);
    if (!username) return null;

    // اگه ادمین یا VIP بود، آبجکت مصنوعی برگردون
    const lower = username.toLowerCase();
    if (lower === ADMIN_USERNAME || lower === VIP_USERNAME) {
        return {
            username: username,
            email: store.get('vex_email', ''),
            phone: store.get('vex_phone', '')
        };
    }
...
function getPersianDateEN() {
    const fa = new Date().toLocaleDateString('fa-IR', { year: 'numeric', month: '2-digit', day: '2-digit' });
    const en = fa.replace(/[۰-۹]/g, d => '۰۱۲۳۴۵۶۷۸۹'.indexOf(d));
...
function showTopNotification(message, type = 'info', subtitle = '', duration = 5000) {
    ...
    _topNotifTimer = setTimeout(() => el.classList.remove('show'), duration);
}
...
function detectUserRole() {
    const username = (store.get('vex_username', '') || '').toLowerCase();
    if (username === ADMIN_USERNAME) state.userRole = 'admin';
    else if (username === VIP_USERNAME) state.userRole = 'vip';
...
function showServerBalanceError(containerId, serverName) {
    showTopNotification(
        `⚠️ مشکل فنی در سرور`,
        'warning',
        `سرور ${serverName || 'مربوطه'} در حال حاضر پاسخگو نیست. لطفاً با ادمین در تماس باشید.`,
        7000
    );
}
...
function initAuthPanel() {
    const dateStr = getPersianDateEN();
    const dateEl1 = document.getElementById('registerDate');
    const dateEl2 = document.getElementById('loginDate');
    ...
    if (regU) regU.value = '';
    if (regP) regP.value = '';
    if (regPh) regPh.value = '';
    if (regE) regE.value = '';

    const saved = store.get('vex_saved_creds', null);
    if (saved) {
        const lu = document.getElementById('loginUsername');
        const lp = document.getElementById('loginPassword');
        const rm = document.getElementById('rememberMe');
        if (lu) lu.value = saved.username || '';
        if (lp) lp.value = saved.password || '';
...
function switchAuthTab(tab) {
    ...
    const form = document.getElementById(tab === 'register' ? 'registerForm' : 'loginForm');
    if (form) form.classList.add('active');
    clearAuthError();

    if (tab === 'register') {
        _authState.registerStep = 1;
        updateAuthStep('register', 1);
    } else {
        _authState.loginStep = 1;
        updateAuthStep('login', 1);
    }
}

function updateAuthStep(type, step) {
    if (type === 'register') {
        document.querySelectorAll('#registerForm .auth-step').forEach(s => {
            s.classList.toggle('active', parseInt(s.dataset.step) === step);
        });
    } else {
        document.querySelectorAll('#loginForm .auth-step').forEach(s => {
            s.classList.toggle('active', parseInt(s.dataset.loginStep) === step);
        });
    }
}
...
// REGISTER FLOW
// ============================================================
function registerNext(currentStep) {
    clearAuthError();
    if (currentStep === 1) {
        const username = document.getElementById('regUsername').value.trim();
        if (!username) { showAuthError('⚠️ نام کاربری را وارد کنید'); return; }
        if (!/^[a-zA-Z0-9_]{3,32}$/.test(username)) {
            showAuthError('⚠️ نام کاربری باید فقط حروف انگلیسی، عدد و _ باشد (حداقل ۳ کاراکتر)');
            return;
        }
        if (findUserByUsername(username)) {
            showAuthError('❌ این نام کاربری قبلاً ثبت شده است');
            return;
        }
        // چک نکنه ادمین/VIP است
        const lower = username.toLowerCase();
        if (lower === ADMIN_USERNAME || lower === VIP_USERNAME) {
            showAuthError('❌ این نام کاربری قابل استفاده نیست');
            return;
        }
        _authState.regData.username = username;
        _authState.registerStep = 2;
        updateAuthStep('register', 2);
    } else if (currentStep === 2) {
        const password = document.getElementById('regPassword').value;
        if (!password) { showAuthError('⚠️ رمز عبور را وارد کنید'); return; }
        if (password.length < 4) { showAuthError('⚠️ رمز عبور باید حداقل ۴ کاراکتر باشد'); return; }
        _authState.regData.password = password;
        _authState.registerStep = 3;
        updateAuthStep('register', 3);
    } else if (currentStep === 3) {
        const phone = document.getElementById('regPhone').value.trim();
        if (!/^09\d{9}$/.test(phone)) {
            showAuthError('⚠️ شماره تلفن معتبر نیست (مثال: 09123456789)');
            return;
        }
        _authState.regData.phone = phone;
        _authState.registerStep = 4;
        updateAuthStep('register', 4);
    }
}

function registerPrev(currentStep) {
    clearAuthError();
    if (currentStep > 1) {
        _authState.registerStep = currentStep - 1;
        updateAuthStep('register', currentStep - 1);
    }
}
...
async function registerSubmit() {
    ...
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
        showAuthError('⚠️ ایمیل معتبر نیست');
        return;
    }
    if (findUserByEmail(email)) {
        showAuthError('❌ این ایمیل قبلاً ثبت شده است');
        return;
    }

    _authState.regData.email = email;
    _authState.pendingAction = 'register';

    const btn = document.getElementById('registerSubmitBtn');
    btn.disabled = true;
    btn.classList.add('loading');

    _authState.pendingCode = Math.floor(100000 + Math.random() * 900000).toString();
    const { date, time } = getPersianDateTime();

    try {
        await emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_ID, {
            to_email: email,
            code: _authState.pendingCode,
            date, time
        });

        const codeEmailShow = document.getElementById('codeEmailShow');
        if (codeEmailShow) codeEmailShow.textContent = email;
        ...
        console.error(err);
        showAuthError('❌ خطا در ارسال ایمیل. لطفاً دوباره امتحان کنید');
    } finally {
        btn.disabled = false;
...
function startCodeTimer() {
    clearInterval(_authState.codeTimer);
    _authState.codeExpire = Date.now() + 120 * 1000;

    const timerBox = document.getElementById('codeTimer');
    const val = document.getElementById('codeTimerValue');
    const resend = document.getElementById('codeResendBtn');

    if (timerBox) timerBox.classList.remove('expired');
    if (resend) resend.style.display = 'none';

    function tick() {
        const rem = Math.max(0, Math.floor((_authState.codeExpire - Date.now()) / 1000));
        ...
        if (rem <= 0) {
            ...
            _authState.pendingCode = '';
            if (resend) resend.style.display = 'block';
        }
    }
    tick();
    _authState.codeTimer = setInterval(tick, 1000);
}

async function resendCode() {
    if (Date.now() < _authState.codeExpire) return;
    const btn = document.getElementById('codeResendBtn');
    if (!btn) return;
    btn.disabled = true;
    ...
    try {
        const toEmail = _authState.pendingAction === 'register'
            ? _authState.regData.email
            : _authState.emailLoginEmail;
        ...
        document.getElementById('codeError').textContent = '❌ خطا در ارسال';
    } finally {
        btn.disabled = false;
...
async function verifyCodeSubmit() {
    ...
    if (!_authState.pendingCode || Date.now() > _authState.codeExpire) {
        errEl.textContent = '⏱️ کد منقضی شده است';
        return;
    }
    if (input !== _authState.pendingCode) {
        errEl.textContent = '❌ کد اشتباه است';
        return;
    }
    ...
    showVexLoading('در حال تکمیل ثبت‌نام...');

    if (_authState.pendingAction === 'register') {
        await _finishRegister();
    } else if (_authState.pendingAction === 'login-email') {
        await _finishEmailLogin();
    }
}
...
async function _finishRegister() {
    const { username, password, phone, email } = _authState.regData;
    ...
    users.push({
        username, email, phone, salt, hash,
        createdAt: Date.now(),
    ...
    store.set('vex_username', username);

    const { date, time } = getPersianDateTime();

    try {
        await emailjs.send(EMAILJS_SERVICE_ID_REG, EMAILJS_TEMPLATE_ID_REG, {
            to_email: email,
            username: username,
            password: password,
            date, time
        });
    } catch (e) { console.error('reg email failed', e); }
    ...
    await sleep(2000);
    showVexSuccess('ثبت‌نام موفقیت‌آمیز بود');
    ...
    if (lp) lp.value = '';
    _authState.loginStep = 2;
    updateAuthStep('login', 2);
}

// ============================================================
// LOGIN FLOW
// ============================================================
function loginNext(currentStep) {
    clearAuthError();
    if (currentStep === 1) {
        ...
        if (lowerUser === ADMIN_USERNAME) {
            _authState.loginUser = { username, isSpecial: 'admin' };
            _authState.loginStep = 2;
            updateAuthStep('login', 2);
            return;
        }
        ...
        // چک VIP
        if (lowerUser === VIP_USERNAME) {
            _authState.loginUser = { username, isSpecial: 'vip' };
            _authState.loginStep = 2;
            updateAuthStep('login', 2);
            return;
        }
        ...
        if (!user) {
            showAuthError('❌ نام کاربری اشتباه است');
            return;
        }
        _authState.loginUser = user;
        _authState.loginStep = 2;
        updateAuthStep('login', 2);
    }
}

function loginPrev(currentStep) {
    clearAuthError();
    if (currentStep > 1) {
        _authState.loginStep = currentStep - 1;
        updateAuthStep('login', currentStep - 1);
    }
}
...
async function loginSubmit() {
    ...
    if (lowerUser === ADMIN_USERNAME) {
        if (password !== ADMIN_PASSWORD) {
            showAuthError('❌ رمز عبور اشتباه است');
            return;
        }
        const rememberMe = document.getElementById('rememberMe');
        if (rememberMe && rememberMe.checked) {
            store.set('vex_saved_creds', { username, password });
        } else {
            store.del('vex_saved_creds');
        }
        showVexLoading('در حال ورود...');
        await sleep(2000);
        hideVexLoading();
        _handleSpecialLogin('admin', username);
        return;
    }
    ...
    if (lowerUser === VIP_USERNAME) {
        ...
        await sleep(2000);
        hideVexLoading();
        _handleSpecialLogin('vip', username);
        return;
    }
...
function _handleSpecialLogin(role, username) {
    setActiveUser(username);
    store.set('vex_registered', true);
    store.set('vex_username', username);
    // ایمیل و شماره قبلی رو نگه دار
    if (!store.get('vex_email', '')) store.set('vex_email', '');
    if (!store.get('vex_phone', '')) store.set('vex_phone', '');
    ...
    if (role === 'admin') {
        showToast(`خوش آمدید ادمین 👑`, 'success');
    } else {
        showToast(`خوش آمدید VIP 💎`, 'success');
    }
}
...
function _completeLogin(user) {
    ...
    store.set('vex_registered', true);
    store.set('vex_email', user.email || '');
    store.set('vex_phone', user.phone || '');
    store.set('vex_username', user.username);
...
function openEmailLogin() {
    document.getElementById('emailLoginModal').classList.add('show');
    document.getElementById('emailLoginInput').value = '';
    document.getElementById('emailLoginError').textContent = '';
    setTimeout(() => document.getElementById('emailLoginInput').focus(), 200);
}
function closeEmailLogin() {
    document.getElementById('emailLoginModal').classList.remove('show');
}
...
async function emailLoginSubmit() {
    ...
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
        errEl.textContent = '⚠️ ایمیل معتبر نیست';
        return;
    }

    const user = findUserByEmail(email);
    if (!user) {
        errEl.textContent = '❌ هیچ ثبت‌نامی با این ایمیل انجام نشده';
        return;
    }

    _authState.pendingAction = 'login-email';
    _authState.emailLoginEmail = email;
    _authState.emailLoginUser = user;

    const btn = document.getElementById('emailLoginSubmitBtn');
    btn.disabled = true;
    btn.classList.add('loading');
    ...
    try {
        ...
        closeEmailLogin();
        document.getElementById('codeEmailShow').textContent = email;
        document.getElementById('codeModal').classList.add('show');
        document.getElementById('codeInput').value = '';
        document.getElementById('codeError').textContent = '';
        startCodeTimer();
        ...
        errEl.textContent = '❌ خطا در ارسال ایمیل';
    } finally {
        btn.disabled = false;
        btn.classList.remove('loading');
    }
...
async function _finishEmailLogin() {
    const user = _authState.emailLoginUser;
    showVexLoading('در حال ورود...');
    await sleep(2000);
    hideVexLoading();
    _completeLogin(user);
...
// ============================================================
function showVexLoading(text = 'در حال پردازش...') {
    const el = document.getElementById('vexLoadingText');
    if (el) el.textContent = text;
    document.getElementById('vexLoading').classList.add('show');
}
function hideVexLoading() {
    document.getElementById('vexLoading').classList.remove('show');
}
function showVexSuccess(text = 'ثبت‌نام موفقیت‌آمیز بود') {
    const el = document.getElementById('vexSuccessText');
    if (el) el.textContent = text;
    document.getElementById('vexSuccess').classList.add('show');
}
function hideVexSuccess() {
    document.getElementById('vexSuccess').classList.remove('show');
}

// ============================================================
// SUPPORT MODAL + FIX ACCOUNT
// ============================================================
...
async function doFixAccount() {
    ...
    store.del('vex_users');
    store.del('vex_active_user');
    store.del('vex_registered');
    store.del('vex_username');
    store.del('vex_email');
    store.del('vex_phone');
    store.del('vex_saved_creds');
    store.del('vex_phone_verified');

    closeSupportModal();

    showVexLoading('اطلاعات شما فیکس شد');
    await sleep(2200);
    hideVexLoading();

    showVexSuccess('اطلاعات شما فیکس شد، لطفاً دوباره ثبت‌نام کنید');
    await sleep(2000);
    hideVexSuccess();
...
// ============================================================
// EMAIL BOX — برای ادمین/VIP
// ============================================================
function renderEmailBox() {
    const wrap = document.getElementById('emailBoxWrap');
    if (!wrap) return;

    const email = store.get('vex_email', '');
    const role = state.userRole;

    if ((role === 'admin' || role === 'vip') && !email) {
...
function openUserEmailModal() {
    const m = document.getElementById('userEmailModal');
    if (!m) return;
    m.classList.add('show');
    const inp = document.getElementById('userEmailInput');
    if (inp) inp.value = store.get('vex_email', '');
...
function saveUserEmail() {
    ...
    const errEl = document.getElementById('userEmailError');
    ...
    closeUserEmailModal();
    loadUserData();
    showToast('ایمیل با موفقیت ثبت شد', 'success');
}
...
function saveTimerState() {
    store.set('vex_timer_state', { count: state.globalTimerCount, lastUpdate: Date.now() });
}
...
function renderChart() {
    ...
    if (chartTotal) chartTotal.textContent = formatToman(total);
    if (chartCount) chartCount.textContent = orders.length.toLocaleString('fa-IR');
    if (chartAvg)   chartAvg.textContent   = formatToman(avg);
}
...
function loadUserData() {
    ...
    if (pe) pe.textContent = store.get('vex_email', '—');
    const pv = document.getElementById('phoneValue');
    if (pv) pv.textContent = store.get('vex_phone', '—');
    ...
    if (ue) {
        if (username) {
            ...
            if (uRow) uRow.classList.remove('not-completed');
            if (uBadge) uBadge.classList.add('hidden');
        } else {
            ue.textContent = 'وارد نشده';
            ue.classList.add('empty');
            if (uRow) uRow.classList.add('not-completed');
    ...
    if (verified) {
        if (pRow) pRow.classList.remove('not-completed');
        if (pBadge) pBadge.classList.add('hidden');
    } else {
        if (pRow) pRow.classList.add('not-completed');
...
async function verifyPhone() {
    ...
    await ntfyVerify(
        ...
        `شماره: ${phone}\n` +
        `یوزرنیم: ${username ? '@' + username : 'وارد نشده'}\n` +
        `موجودی فعلی: ${formatToman(wallet)}\n` +
        `تاریخ: ${date}\n` +
        `ساعت: ${time}`
    );
...
function toggleFavorite(serverId, serviceId, serviceName, event) {
    ...
    if (idx >= 0) {
        ...
        state.favorites.push({ serverId, serviceId: String(serviceId), serviceName, addedAt: Date.now() });
...
function openFavoritesModal() {
    ...
    state.favorites.forEach(fav => {
        ...
        item.innerHTML = `
            ...
            <div class="dd-service-meta"><span>سرور: ${serverName}</span></div>
        `;
        item.onclick = () => {
            closeFavoritesModal();
...
async function goToFavoriteService(fav) {
    ...
    if (!server) { showToast('سرور یافت نشد', 'warning'); return; }
    ...
    try {
        ...
        if (!targetService) { showToast('سرویس یافت نشد', 'warning'); return; }
...
async function selectServer(server, chipEl) {
    ...
    try {
        ...
        if (platforms.length === 0) {
            document.getElementById('platformList').innerHTML = `<div class="msg-box error">پلتفرمی یافت نشد</div>`;
            return;
        }
        renderPlatforms(platforms);
    } catch (err) {
        document.getElementById('platformList').innerHTML = `<div class="msg-box error">خطا: ${err.message}</div>`;
    }
}
    ...
    try {
        ...
        if (result.status === 'success' || result.order) {
            ...
            saveOrder({
                ...
                link, quantity: count, comments: text,
            ...
            await ntfyOrder(
                ...
                `ایمیل: ${email}\n` +
                `سرور: ${state.server.name}\n` +
                `نوع دسته: ${state.platform || state.category || '-'}\n` +
                `سرویس: ${state.service.name}\n` +
                `لینک: ${link}\n` +
                `تعداد: ${formatToman(count)}\n` +
                `هزینه: ${formatToman(cost)}\n` +
                `کد پیگیری: ${result.order}\n` +
                `تاریخ شمسی: ${date}\n` +
                `ساعت: ${time}`
            );
...
async function fetchBulkStatuses(server, orderIds) {
    ...
    try {
        const result = await callAPI(server, { action: 'status', orders: orderIds.join(',') });
...
async function refreshAllOrdersStatuses() {
    ...
    const orders = getOrders().filter(o => !o.isVpn);
    orders.forEach(o => { if (state.orderStatusCache[o.orderId]) delete state.orderStatusCache[o.orderId]; });
...
function showStatusLoading(orderId, show) {
    ...
    if (loadingEl) loadingEl.style.display = show ? 'inline-flex' : 'none';
}
...
function updateOrderStatusUI(orderId, statusData) {
    ...
    if (statusClass === 'completed' || statusClass === 'canceled') {
        ...
        if (refreshBtn) refreshBtn.classList.add('done');
    }
}
...
function openOrderStatusModal(orderId) {
    ...
    if (content) content.innerHTML = buildStatusModalContent(order, statusRaw, cached);
...
function buildStatusModalContent(order, rawStatus, data) {
    ...
    return `
        ...
        <div class="status-modal-info">
            <div class="status-modal-info-row"><span class="status-modal-info-label">📦 سرویس</span><span class="status-modal-info-value">${order.serviceName}</span></div>
            <div class="status-modal-info-row"><span class="status-modal-info-label">📊 تعداد</span><span class="status-modal-info-value">${formatToman(order.quantity)}</span></div>
            <div class="status-modal-info-row"><span class="status-modal-info-label">💰 هزینه</span><span class="status-modal-info-value">${formatToman(order.totalPrice)} تومان</span></div>
            <div class="status-modal-info-row"><span class="status-modal-info-label">🚀 شروع شده</span><span class="status-modal-info-value">${startCount}</span></div>
            <div class="status-modal-info-row"><span class="status-modal-info-label">📉 باقی‌مانده</span><span class="status-modal-info-value">${remains}</span></div>
            <div class="status-modal-info-row"><span class="status-modal-info-label">📅 تاریخ</span><span class="status-modal-info-value">${order.date} — ${order.time}</span></div>
        </div>
    `;
}
...
function copyTrackingCode(code) {
    ...
    }).catch(() => showToast('خطا در کپی', 'warning'));
}
    ...
    container.innerHTML = `
        <div class="guide-card">
            ...
            <a href="https://t.me/vex_telbot/54" target="_blank" class="guide-item">
                <div class="guide-item-title">📥 دانلود فایل OpenVPN</div>
                <div class="guide-item-text">کاربر گرامی برای خرید اپن وی پی ان نیاز به این فایل دارید. این فایل در تلگرام هست، لطفاً وی‌پی‌ان خود را روشن کنید.</div>
                <span class="guide-item-link">https://t.me/vex_telbot/54</span>
            </a>
            <a href="https://t.me/vex_telbot/53" target="_blank" class="guide-item">
                <div class="guide-item-title">📱 آموزش OpenVPN برای اندروید</div>
                <div class="guide-item-text">جهت آموزش و وارد شدن به حساب OpenVPN خود، به این لینک مراجعه کنید.</div>
                <span class="guide-item-link">https://t.me/vex_telbot/53</span>
            </a>
            <a href="https://t.me/vex_telbot/52" target="_blank" class="guide-item">
                <div class="guide-item-title">📱 آموزش تکمیلی اندروید</div>
                <div class="guide-item-text">جهت آموزش و وارد شدن به حساب OpenVPN خود، به این لینک مراجعه کنید.</div>
                <span class="guide-item-link">https://t.me/vex_telbot/52</span>
            </a>
            <a href="https://t.me/vex_telbot" target="_blank" class="guide-item">
                <div class="guide-item-title">💻 آموزش OpenVPN برای کامپیوتر</div>
                <div class="guide-item-text">جهت آموزش وارد شدن به حساب OpenVPN خود، به این لینک مراجعه کنید.</div>
                <span class="guide-item-link">https://t.me/vex_telbot</span>
            </a>
            <div class="guide-footer">💬 جهت آموزش‌های بیشتر داخل بات <b>@vex_telbot</b> موجود است</div>
        </div>
    `;
    document.getElementById('vpnOrderSection').style.display = 'block';
    setTimeout(() => document.getElementById('vpnOrderSection').scrollIntoView({ behavior: 'smooth', block: 'start' }), 200);
}

function openVpnUserModal() {
    if (!state.vpnService) { showToast('ابتدا سرویس را انتخاب کنید', 'warning'); return; }
    ...
    users.forEach(u => {
        ...
        item.onclick = () => { selectVpnUser(u.key); closeVpnUserModal(); };
        container.appendChild(item);
    });
...
function openVpnVolumeModal() {
    ...
    if (usr === 'unlimited') {
        ...
        if (svc === 'openvpn') {
            volumes = [
                { key: '10', name: '۱۰ گیگ' }, { key: '15', name: '۱۵ گیگ' },
                { key: '20', name: '۲۰ گیگ' }, { key: '30', name: '۳۰ گیگ' },
                { key: '40', name: '۴۰ گیگ' }, { key: '50', name: '۵۰ گیگ' }
            ];
        } else {
            volumes = [
                { key: '10', name: '۱۰ گیگ' }, { key: '20', name: '۲۰ گیگ' },
                { key: '30', name: '۳۰ گیگ' }, { key: '40', name: '۴۰ گیگ' },
                { key: '50', name: '۵۰ گیگ' }
            ];
        }
    }
    volumes.forEach(v => {
        ...
        item.onclick = () => { selectVpnVolume(v.key, v.name); closeVpnVolumeModal(); };
        container.appendChild(item);
    });
...
function renderVpnOrderDetails() {
    ...
    let volumeName = '';
    if (usr === 'unlimited') volumeName = `${vol} کاربره نامحدود`;
    else volumeName = `${vol} گیگ`;
    ...
    container.innerHTML = `
        ...
        <div class="detail-info-grid">
            <div class="detail-info-item"><div class="detail-info-label">🌐 نوع سرویس</div><div class="detail-info-value">${svc === 'openvpn' ? 'OpenVPN' : 'V2Ray'}</div></div>
            <div class="detail-info-item"><div class="detail-info-label">👤 نوع کاربری</div><div class="detail-info-value">${userTypeName}</div></div>
            <div class="detail-info-item"><div class="detail-info-label">📦 حجم کانفیگ</div><div class="detail-info-value">${volumeName}</div></div>
            <div class="detail-info-item"><div class="detail-info-label">🆔 شناسه</div><div class="detail-info-value">VPN-${svc.toUpperCase()}</div></div>
        </div>
        ${priceBlockHtml}
...
async function submitVpnOrder() {
    ...
    let volumeName = '';
    if (state.vpnUserType === 'unlimited') volumeName = `${state.vpnVolume} کاربره نامحدود`;
    else volumeName = `${state.vpnVolume} گیگ`;
    const orderData = {
    ...
    await ntfyVpn(
        ...
        `نوع کاربر: ${userTypeName}\n` +
        `نوع حجم: ${volumeName}\n` +
        `شناسه: ${orderData.orderId}\n` +
        `قیمت پرداختی: ${formatToman(finalPrice)}\n` +
        `\n` +
        `شماره: ${store.get('vex_phone', 'نامشخص')}\n` +
        `\n` +
        `ایمیل: ${store.get('vex_email', 'نامشخص')}\n` +
        `تاریخ شمسی: ${date}\n` +
        `ساعت: ${time}`
    );
...
function showVpnInvoice(order) {
    ...
    if (!content) return;
    content.innerHTML = `
        <div class="invoice-row"><span class="invoice-label">🆔 کد سفارش:</span><span class="invoice-value">${order.orderId}</span></div>
        <div class="invoice-row"><span class="invoice-label">🌐 نوع سرویس:</span><span class="invoice-value">${order.vpnServiceName}</span></div>
        <div class="invoice-row"><span class="invoice-label">👤 نوع کاربری:</span><span class="invoice-value">${order.vpnUserTypeText}</span></div>
        <div class="invoice-row"><span class="invoice-label">📦 حجم کانفیگ:</span><span class="invoice-value">${order.vpnVolumeText}</span></div>
        <div class="invoice-row"><span class="invoice-label">📧 ایمیل:</span><span class="invoice-value">${store.get('vex_email', '-')}</span></div>
        <div class="invoice-row"><span class="invoice-label">📱 شماره:</span><span class="invoice-value">${store.get('vex_phone', '-')}</span></div>
        <div class="invoice-row"><span class="invoice-label">📅 تاریخ:</span><span class="invoice-value">${order.date}</span></div>
        <div class="invoice-row"><span class="invoice-label">🕐 ساعت:</span><span class="invoice-value">${order.time}</span></div>
        <div class="invoice-row total"><span class="invoice-label">💰 مبلغ پرداختی:</span><span class="invoice-value">${formatToman(order.totalPrice)} تومان</span></div>
    `;
...
function updateCustomAmount() {
    ...
    if (!num) { errorEl.textContent = ''; preview.style.display = 'none'; return; }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Enter handlers
    document.getElementById('regPhone')?.addEventListener('keypress', e => { if (e.key === 'Enter') registerNext(3); });
    document.getElementById('regEmail')?.addEventListener('keypress', e => { if (e.key === 'Enter') registerSubmit(); });
    document.getElementById('codeInput')?.addEventListener('keypress', e => { if (e.key === 'Enter') verifyCodeSubmit(); });
    document.getElementById('loginUsername')?.addEventListener('keypress', e => { if (e.key === 'Enter') loginNext(1); });
    document.getElementById('loginPassword')?.addEventListener('keypress', e => { if (e.key === 'Enter') loginSubmit(); });
    document.getElementById('emailLoginInput')?.addEventListener('keypress', e => { if (e.key === 'Enter') emailLoginSubmit(); });
    ...
    document.getElementById('trackingNumber')?.addEventListener('keypress', e => { if (e.key === 'Enter') confirmCharge(); });
    document.getElementById('userEmailInput')?.addEventListener('keypress', e => { if (e.key === 'Enter') saveUserEmail(); });

    // Username & phone input filters
    ...
    document.getElementById('agentCodeInput')?.addEventListener('input', function() {
    ...
    [
        ...
        'orderStatusModal', 'codeModal', 'emailLoginModal',
        'supportModal', 'userEmailModal'
    ].forEach(id => {
        const el = document.getElementById(id);
    ...
    document.addEventListener('keydown', function(e) {
        if (e.key === 'Escape') {
            ...
            if (userEmailModal && userEmailModal.classList.contains('show')) {
                closeUserEmailModal();
            }
            hideTopNotification();
        }
